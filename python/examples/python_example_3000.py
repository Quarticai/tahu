...
# "spBv1.0/GroupID123/DDATA/Edge_Node_1/Emulated_Device/#"
# Application Variables
serverUrl = "138.91.251.188"
serverUrl = "localhost"
myGroupId = "group1"
myNodeName = "node1"
myDeviceName = "device1"
publishPeriod = 5000
myUsername = "admin"    
myPassword = "changeme"
...
def on_connect(client, userdata, flags, rc):
    ...
    # client.subscribe("spBv1.0/" + myGroupId + "/DDATA/" + myNodeName + "/#")
...
def on_message(client, userdata, msg):
    ...
    if tokens[0] == "spBv1.0" and tokens[1] == myGroupId and (tokens[2] == "NCMD" or tokens[2] == "DCMD") and tokens[3] == myNodeName:
        ...
        for metric in inboundPayload.metrics:
            for i in range(len(metric.properties.keys)):
                print(metric.properties.keys[i], metric.properties.values[i])
...
def publishDeviceBirth():
    ...
    metric = addMetric(payload, "Temperature Metric", None, MetricDataType.DateTime, int(time.time() * 1000))
    # metric.properties.keys.extend(["Quality"])
    # propertyValue = metric.properties.values.add()
    # propertyValue.type = ParameterDataType.Double
    # propertyValue.double_value = 500
    for i in range(3000):
        metric.properties.keys.extend([f"Temperature{i}"])
        propertyValue = metric.properties.values.add()
        propertyValue.type = ParameterDataType.Double
        propertyValue.double_value = 98.7
    # metric.properties.keys.extend(["Pressure"])
    # propertyValue = metric.properties.values.add()
    # propertyValue.type = ParameterDataType.String
    # propertyValue.string_value = 1.5
    # metric.properties.keys.extend(["Valu"])
    # propertyValue = metric.properties.values.add()
    # propertyValue.type = ParameterDataType.String
    # propertyValue.string_value = "1 s"
    # Create the UDT definition value which includes two UDT members and a single parameter and add it to the payload
    template = initTemplateMetric(payload, "My_Custom_Motor", None, "Custom_Motor")
    templateParameter = template.parameters.add()
    templateParameter.name = "Index"
    templateParameter.type = ParameterDataType.Double
    templateParameter.double_value = 1
    ...
    client.publish("spBv1.0/" + myGroupId + "/DBIRTH/" + myNodeName + "/" + 'device2', totalByteArray, 0, False)
...
while True:
    ...
    metric = addMetric(payload, "Temperature Metric", None, MetricDataType.Boolean, random.choice([True, False]), timestamp=int(time.time()*1000))
    # metric.properties.keys.extend(["Quality"])
    # propertyValue = metric.properties.values.add()
    # propertyValue.type = ParameterDataType.Double
    # propertyValue.double_value = 500
    ...
    for i in range(3000):
        ...
        propertyValue = metric.properties.values.add()
        propertyValue.type = ParameterDataType.Double
        propertyValue.double_value = random.choice([98.7, 98.6, 105, 108, 90, 56, 91, 45, 26, 100])
    # print(f"Temperature {propertyValue.double_value} sent")
    # metric.properties.keys.extend(["Pressure"])
    # propertyValue = metric.properties.values.add()
    # propertyValue.type = ParameterDataType.Double
    # propertyValue.double_value = 1.5
    # metric.properties.keys.extend(["Valu"])
    # propertyValue = metric.properties.values.add()
    # propertyValue.type = ParameterDataType.Double
    # propertyValue.double_value = 1
    # print(metric.timestamp)
    # Publish a message data
    byteArray = bytearray(payload.SerializeToString())
    client.publish("spBv1.0/" + myGroupId + "/DDATA/" + myNodeName + "/" + myDeviceName, byteArray, 0, False)
    # client.publish("spBv1.0/" + myGroupId + "/DDATA/" + myNodeName + "/" + 'device2', byteArray, 0, False)
    # print('sent data')
    time.sleep(.5)
